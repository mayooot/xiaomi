...
<div id="outer">
    <header>
        ...
        <div id="personinfo">
            <div><span id="username">未登录</span><i class="glyphicon glyphicon-chevron-down"></i>
                <ul>
                    <li>退出</li>
                </ul>
            </div>
...
<div id="cartcontainer">
    ...
    <div class="list-order">
        <div class="shop">
            <a href="index.jsp" style="color: orangered">继续购物 </a>
            <span>共 <strong id="goods_count">0</strong> 件商品，已选择 <strong>0</strong> 件</span>
        </div>
        <div class="order-res">
            <span>合计：<strong id="goods_money">0</strong>元</span>
...
<script>
    ...
    $(function () {
        ...
        $.ajax({
            url:"/xiaomi/shopCart/findAll",
            method: "GET",
            success: function (response) {
                console.log(response);

                let $goods_money = 0;
                let $cartList = response.objectMap.goodsCartList;

                // 设置用户名
                let $consumer = response.objectMap.consumer;
                ...
                for(let i = 0; i < $cartList.length; i++) {
                    ...
                    let $checkbox = $("<input>").attr("type", "checkbox").attr("id", "chekList");
                    ...
                    let $col_price = $("<div>").addClass("col-price").text($cartList[i].subtotal / $cartList[i].buyCount);
                    ...
                    $goods_money += $cartList[i].subtotal;
                    let $operation = $("<div>").addClass("col-action").append($("<span>").addClass("glyphicon glyphicon-remove").attr("id", "delGoods"));
                    ...
                    // 设置商品总数
                    $("#goods_count").text($cartList.length);

                    // 设置商品总价
                    $("#goods_money").text($goods_money);
                }
            },
            error: function () {
                console.log("请求失败");
