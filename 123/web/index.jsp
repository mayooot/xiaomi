...
<div id="top">
    <div id="navtop">
        ...
        <div class="ntright">
            <span id="hello"></span>
            <span style="display: none" id="exit"><a href="/xiaomi/consumer/exit">退出</a></span>
            <a id="a-login" href="login.jsp">登录</a> | <a id="a-register" href="register.jsp">注册</a> | <a href="#">消息通知</a>  <div><a href="/xiaomi/shopcart.jsp">购物车 <span id="shopCartSize">(0)</span></a></div>
        </div>
    </div>
</div>
<header>
    ...
    $(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // 如果已经登录则隐藏登录、注册。显示欢迎您，xxx
                let $consumer = response.objectMap.consumer;
                if ($consumer != null) {
                    // 用户已经登录 隐藏登录、注册按钮 显示退出按钮
                    $("#a-login").css("display", "none");
                    $("#a-register").css("display", "none");
                    $("#hello").text("欢迎您，" + $consumer.username).css("color", "rgb(255,245,246)").css("font-size", "15px");
                    $("#exit").css("display", "")
                }
                // 设置右上角购物车的数量
                let $shopCartSize = response.objectMap.shopCartSize;
                $("#shopCartSize").text('(' + $shopCartSize + ')').css("color", "rgb(255,245,246)");
                // 加载楼层商品
                // 获取到后台返回的一级商品类型数据
                // 遍历商品类型
        ...
        function loadGoods(goodsList, container) {
            ...
            for (let i = 0; i < goodsList.length; i++) {
                ...
                let $desc = $("<div>").addClass("desc").text(goodsList[i].remark.substring(0, 16));
